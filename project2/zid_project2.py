...
def mk_ret_df(prc_df):
    ...
    ff_df = pd.read_csv(cfg.FF_CSV)
    ff_df = ff_df[['Date', 'mkt']]
    ff_df['Date'] = pd.to_datetime(ff_df['Date'])
    ff_df.set_index('Date', inplace=True)
    ret_df = pd.DataFrame(index=prc_df.index)

    for col in prc_df.columns:
        ret_df[col] = prc_df[col].pct_change()

    ret_df['mkt_ret'] = ff_df['mkt'].pct_change()
